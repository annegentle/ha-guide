...
<xsl:template name="process.image">
  ...
  <xsl:variable name="filename.for.graphicsize">
    <xsl:choose>
      ...
      <xsl:when test="not(starts-with($filename, '/')) and not(contains($filename, '://')) and ancestor-or-self::*/@xml:base">
        <xsl:value-of select="concat(ancestor-or-self::*/@xml:base[1], $filename)"/>
      </xsl:when>
      <xsl:otherwise>
	<xsl:value-of select="$filename"/>
      </xsl:otherwise>
